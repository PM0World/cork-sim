...
amm_eth = st.number_input(
    "AMM ETH liquidity",
    min_value=10_000.0,
    max_value=1_000_000.0,
    value=50_000.0,        # default ≥ min (fixes StreamlitValueBelowMinError)
    step=5_000.0,
)

amm_token = st.number_input(
    "AMM token liquidity",
    min_value=10_000.0,
    max_value=1_000_000.0,
    value=50_000.0,        # default ≥ min
    step=5_000.0,
)
...
if st.button("▶️ Run simulation"):
    cfg = dict(
        ...
        blocks      = blocks,
        eth_yield   = 0.00001,
        lst_yield   = lst_apr / 100 / 365,   # convert APR → per-block
        amm_eth     = amm_eth,
        amm_token   = amm_token,
        amm_fee     = amm_fee,
    )
    ...
    minutes  = res["summary"]["blocks_under_peg"] * 12 / 60  # 12 s/block
    st.success(
        f"Min price {1-min_pct:.3f} (−{min_pct:.1%}); "
        f"time under peg ≈ {minutes:.1f} min"
    )
    ...
    # ── final wallet face value bar chart ───────────────────────────
    final_bal = (
